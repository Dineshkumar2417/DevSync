...
const Login = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // LOG EVERYTHING to your console so you can see the fix working
      console.log("Server Response:", res.data);

      const token = res.data.token;
      // Some backends use ._id, some use .id, some use .userId
      const userId = res.data.userId || res.data.id || res.data.user?._id;

      if (token && userId) {
        localStorage.setItem('token', token);
        localStorage.setItem('userId', userId);
        
        // Use a slight delay to ensure storage is saved before the Dashboard runs its check
        setTimeout(() => {
          navigate('/dashboard');
        }, 200);
      } else {
      ...
      <div className="w-full max-w-md bg-white/[0.03] backdrop-blur-xl border border-white/10 rounded-[3rem] p-10 shadow-2xl relative overflow-hidden">
        ...
        <form onSubmit={handleSubmit} className="space-y-4 relative">
          <div className="relative">
            ...
            <input type="email" placeholder="EMAIL ADDRESS" required className="w-full bg-slate-950 border border-white/5 text-white p-4 pl-12 rounded-2xl outline-none focus:border-blue-500/50 transition-all placeholder:text-slate-700" onChange={(e) => setFormData({...formData, email: e.target.value})} />
          </div>
          <div className="relative">
            <Lock className="absolute left-4 top-4 text-slate-500" size={18} />
            <input type="password" placeholder="PASSWORD" required className="w-full bg-slate-950 border border-white/5 text-white p-4 pl-12 rounded-2xl outline-none focus:border-blue-500/50 transition-all placeholder:text-slate-700" onChange={(e) => setFormData({...formData, password: e.target.value})} />
          </div>
