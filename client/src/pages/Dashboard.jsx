...
const Dashboard = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      // Faster: Fetch both User and Projects at the same time
  ...
  const handleFormSubmit = async (e) => {
    e.preventDefault();
    setIsSubmitting(true); // Button feedback starts
    const userId = localStorage.getItem('userId');
    ...
    try {
      ...
      await fetchData(); 
        ...
        alert("Action failed. Check your connection or backend routes."); 
    } finally {
        setIsSubmitting(false); // Button feedback ends
    }
  };
    ...
    return (
      <div className="min-h-screen bg-[#020617] flex flex-col items-center justify-center gap-6">
        <div className="relative flex items-center justify-center">
          <div className="absolute w-20 h-20 border-4 border-blue-600/20 rounded-full animate-pulse"></div>
          <Loader2 className="w-10 h-10 text-blue-600 animate-spin" />
        </div>
        <p className="text-blue-500 font-bold uppercase tracking-[0.4em] text-[10px] animate-pulse">Syncing Workspace</p>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-[#020617] flex text-slate-300 font-sans relative">
      ...
      <aside className={`fixed inset-y-0 left-0 z-40 w-64 border-r border-white/5 bg-[#020617]/95 backdrop-blur-xl p-6 flex flex-col transition-transform duration-300 lg:translate-x-0 ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        <div className="flex items-center justify-between mb-10 text-white px-2">
          <div className="flex items-center gap-3">
            ...
            <span className="text-2xl font-bold uppercase tracking-widest italic italic">DevSync</span>
          </div>
          <button onClick={() => setIsSidebarOpen(false)} className="lg:hidden text-slate-500"><X size={24}/></button>
        </div>
        <nav className="flex-1 space-y-2">
          <button className="w-full flex items-center gap-3 px-4 py-3 rounded-xl bg-blue-600 text-white font-bold shadow-lg shadow-blue-900/20 transition-transform active:scale-95"><Layout size={20}/> Overview</button>
          <button onClick={() => navigate('/profile')} className="w-full flex items-center gap-3 px-4 py-3 rounded-xl text-slate-400 hover:bg-white/5 transition-all active:scale-95"><User size={20}/> My Profile</button>
        </nav>
      ...
      <main className="flex-1 lg:ml-64 p-4 md:p-10 w-full overflow-x-hidden">
        <header className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 mb-12">
          <div className="flex items-center gap-4">
            <button onClick={() => setIsSidebarOpen(true)} className="lg:hidden p-3 bg-white/5 rounded-xl border border-white/10 text-white active:scale-90 transition-transform"><Menu size={24} /></button>
            <div>
        ...
        {/* 2. RESTORED ANALYTICS GRAPHS */}
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-12">
          <div className="bg-white/[0.03] border border-white/10 rounded-[2.5rem] p-8 h-[350px] shadow-inner">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {projects.map((project) => (
            <div key={project._id} className="bg-white/[0.03] border border-white/5 rounded-[2.5rem] overflow-hidden group flex flex-col shadow-xl relative transition-all duration-500 hover:scale-[1.02] hover:border-blue-500/30">
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/80 backdrop-blur-xl p-4">
          <div className="bg-slate-900 border border-white/10 w-full max-w-lg rounded-[3rem] p-8 md:p-10 shadow-2xl overflow-y-auto max-h-[90vh]">
            ...
            <form onSubmit={handleFormSubmit} className="space-y-4">
              ...
              {/* 3. BUTTON FEEDBACK (Reacts when clicked) */}
              <button 
                type="submit" 
