...
import { 
  ...
  CheckCircle2, Image as ImageIcon, Github, ExternalLink, Loader2, Radar, Upload, Menu // Added Menu
} from 'lucide-react';
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-[#020617] flex text-slate-900 dark:text-slate-300 transition-colors relative overflow-x-hidden">
      ...
      <button 
        ...
        className="lg:hidden fixed top-6 left-6 z-50 p-3 bg-blue-600 text-white rounded-2xl shadow-xl shadow-blue-500/20 active:scale-95 transition-all"
      >
        <Menu size={20} />
      </button>
      ...
      `}>
        ...
        <nav className="flex-1 space-y-3">
          <button onClick={() => setIsSidebarOpen(false)} className="w-full flex items-center gap-4 px-5 py-4 rounded-2xl bg-blue-600 text-white font-bold transition-all shadow-lg shadow-blue-500/20">
            <Layout size={20}/> Overview
          </button>
          <button onClick={() => { setIsSidebarOpen(false); navigate('/profile'); }} className="w-full flex items-center gap-4 px-5 py-4 rounded-2xl hover:bg-slate-100 dark:hover:bg-white/5 transition-all font-bold">
      ...
      <main className="flex-1 lg:ml-72 p-6 md:p-12 w-full animate-in fade-in duration-700">
        <header className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 mb-16 mt-12 lg:mt-0">
          ...
          <button onClick={() => { setEditingProjectId(null); setIsModalOpen(true); }} className="w-full md:w-auto bg-blue-600 dark:bg-white text-white dark:text-black font-black px-10 py-5 rounded-[2rem] flex items-center justify-center gap-3 hover:opacity-90 transition-all active:scale-95 shadow-2xl shadow-blue-500/25 dark:shadow-none uppercase text-xs tracking-widest">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 mb-16">
          <div className="bg-white dark:bg-white/[0.02] border border-slate-200 dark:border-white/5 rounded-[3rem] p-10 h-[380px] shadow-sm relative overflow-hidden">
            <h4 className="dark:text-white text-slate-900 font-black mb-8 text-[11px] uppercase tracking-[0.15em] flex items-center gap-3 opacity-70">
              <CheckCircle2 size={18} className="text-blue-500" /> Project Status
            </h4>
            <ResponsiveContainer width="100%" height="80%">
              <PieChart>
                ...
                <Tooltip contentStyle={{ backgroundColor: isDarkMode ? '#0f172a' : '#fff', border: 'none', borderRadius: '20px', fontSize: '12px', fontWeight: 'bold' }} />
              </PieChart>
            </ResponsiveContainer>
          </div>
          <div className="hidden lg:flex bg-white dark:bg-white/[0.02] border border-slate-200 dark:border-white/5 rounded-[3rem] p-10 h-[380px] items-center justify-center italic text-slate-400 dark:text-slate-600 text-[11px] uppercase font-black tracking-[0.3em] shadow-sm">
          ...
          {projects.map((p) => (
            <div key={p._id} className="bg-white dark:bg-white/[0.02] border border-slate-200 dark:border-white/5 rounded-[3rem] overflow-hidden group flex flex-col shadow-2xl transition-all duration-500 hover:scale-[1.03] relative">
              ...
              <div className="absolute top-6 right-6 flex gap-2 opacity-0 group-hover:opacity-100 transition-all z-10 translate-y-2 group-hover:translate-y-0">
                <button onClick={() => handleEditClick(p)} className="p-3 bg-white/10 backdrop-blur-md text-blue-500 rounded-2xl hover:bg-blue-500 hover:text-white transition-all shadow-xl">
                  <Edit3 size={16} />
                </button>
                <button onClick={() => handleDelete(p._id)} className="p-3 bg-white/10 backdrop-blur-md text-red-500 rounded-2xl hover:bg-red-500 hover:text-white transition-all shadow-xl">
                  <Trash2 size={16} />
                </button>
              </div>
              ...
              <div className="h-56 bg-slate-100 dark:bg-slate-950/60 flex items-center justify-center border-b border-slate-200 dark:border-white/5 overflow-hidden">
                {p.thumbnail ? (
                  ...
                  <ImageIcon size={48} className="text-slate-300 dark:text-slate-800 opacity-20" />
                )}
              </div>
              <div className="p-10 grow flex flex-col">
                ...
                <div className="grid grid-cols-2 gap-4 mt-auto font-mono">
                  <a href={p.githubUrl} target="_blank" rel="noreferrer" className="bg-slate-100 dark:bg-white/5 text-center py-4 rounded-2xl text-[11px] font-black border border-slate-200 dark:border-white/5 hover:bg-blue-600 hover:text-white transition-all uppercase tracking-widest">Code</a>
                  <a href={p.liveUrl} target="_blank" rel="noreferrer" className="bg-blue-600 text-white text-center py-4 rounded-2xl text-[11px] font-black shadow-xl hover:opacity-90 transition-all uppercase tracking-widest">Demo</a>
                </div>
              </div>
            </div>
          ))}
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/90 backdrop-blur-xl p-6 animate-in fade-in duration-300">
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-white/10 w-full max-w-xl rounded-[4rem] p-12 shadow-2xl overflow-y-auto max-h-[90vh] animate-in zoom-in-95 relative">
            <div className="flex justify-between items-center mb-10">
              <h3 className="text-3xl font-black dark:text-white text-slate-900 uppercase italic tracking-tighter">
            ...
            <form onSubmit={handleFormSubmit} className="space-y-6">
              ...
              <div className="relative group border-2 border-dashed border-slate-200 dark:border-white/10 bg-slate-50 dark:bg-white/5 hover:border-blue-500/50 rounded-[2.5rem] p-10 text-center transition-all cursor-pointer">
                ...
                <p className="text-[11px] font-black text-slate-500 uppercase tracking-widest">{imageFile ? imageFile.name : 'Upload New Thumbnail'}</p>
              </div>

              <input type="text" required placeholder="Project Title" className="w-full bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-white/10 dark:text-white p-5 rounded-2xl outline-none focus:border-blue-500/50 font-bold" value={projectData.title} onChange={(e) => setProjectData({...projectData, title: e.target.value})} />
              <textarea placeholder="Tell us about the project..." className="w-full bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-white/10 dark:text-white p-5 rounded-2xl outline-none h-32 text-sm focus:border-blue-500/50 font-medium" value={projectData.description} onChange={(e) => setProjectData({...projectData, description: e.target.value})} />
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <input type="url" placeholder="GitHub Repository URL" className="bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-white/10 dark:text-white p-5 rounded-2xl text-xs font-bold" value={projectData.githubUrl} onChange={(e) => setProjectData({...projectData, githubUrl: e.target.value})} />
                <input type="url" placeholder="Live Deployment URL" className="bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-white/10 dark:text-white p-5 rounded-2xl text-xs font-bold" value={projectData.liveUrl} onChange={(e) => setProjectData({...projectData, liveUrl: e.target.value})} />
              </div>
              <button type="submit" disabled={isSubmitting} className="w-full bg-blue-600 dark:bg-white text-white dark:text-black font-black py-6 rounded-[2rem] shadow-2xl mt-6 uppercase text-xs tracking-[0.2em] flex items-center justify-center gap-3 active:scale-95 disabled:opacity-50 transition-all">
                {isSubmitting ? <Loader2 className="w-5 h-5 animate-spin" /> : editingProjectId ? "Update Project" : "Deploy to Portfolio"}
              </button>
            </form>
          </div>
        </div>
