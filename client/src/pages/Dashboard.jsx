...
import { 
  ...
  CheckCircle2, Image as ImageIcon, Github, ExternalLink, Loader2, Radar, Upload, Menu 
} from 'lucide-react';
...
const Dashboard = () => {
  ...
  // 🔥 THEME PERSISTENCE LOGIC: Pehle memory se check karega
  const [isDarkMode, setIsDarkMode] = useState(() => {
    const savedTheme = localStorage.getItem('theme');
    return savedTheme === 'light' ? false : true; // Default dark rahega
  });
    ...
    <div className="min-h-screen bg-slate-50 dark:bg-[#020617] flex text-slate-900 dark:text-slate-300 transition-colors relative overflow-x-hidden">
      ...
      `}>
        ...
        <nav className="flex-1 space-y-3">
          ...
          <button onClick={toggleTheme} className="w-full flex items-center gap-4 px-5 py-4 rounded-2xl hover:bg-slate-100 dark:hover:bg-white/5 transition-all text-slate-500 font-bold">
      ...
      <main className="flex-1 lg:ml-72 p-6 md:p-12 w-full animate-in fade-in duration-700">
        <header className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 mb-16 mt-12 lg:mt-0">
          ...
          <button onClick={() => { setEditingProjectId(null); setIsModalOpen(true); }} className="w-full md:w-auto bg-blue-600 dark:bg-white text-white dark:text-black font-black px-10 py-5 rounded-[2rem] flex items-center justify-center gap-3 hover:opacity-90 transition-all active:scale-95 shadow-2xl shadow-blue-500/25 uppercase text-xs tracking-widest">
          ...
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-white/10 w-full max-w-xl rounded-[4rem] p-12 shadow-2xl overflow-y-auto max-h-[90vh] animate-in zoom-in-95 relative">
            ...
            <form onSubmit={handleFormSubmit} className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <input type="url" placeholder="GitHub Repository" className="bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-white/10 dark:text-white p-5 rounded-2xl text-xs font-bold" value={projectData.githubUrl} onChange={(e) => setProjectData({...projectData, githubUrl: e.target.value})} />
                <input type="url" placeholder="Live Demo" className="bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-white/10 dark:text-white p-5 rounded-2xl text-xs font-bold" value={projectData.liveUrl} onChange={(e) => setProjectData({...projectData, liveUrl: e.target.value})} />
              </div>
